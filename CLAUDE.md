...
```
Read: docs/security/SECURITY_GATES.md (audit checklist)
...
### 3. Recent Security Hardening (2026-01-15 to 2026-01-18)
...
- PR-6: Cookie security hardening (SameSite=Lax, HttpOnly=true, Secure configurable)

### 4. Full Audit Macro Available
```
Run: Read .agent/macros/run-audit.md and execute it
```
Launches 5 parallel agents to generate `docs/security/RECOMMENDATIONS.md`.

### 5. Skills & Ansible
**Agent skills location:** `D:/claude-skills/skills/`
**Macro location:** `.agent/macros/`
**Ansible playbooks:** `ansible/` (staging, production, airgap inventories)
...
### Quick Reference
| Profile | Auth Mode | Credentials |
|---------|-----------|-------------|
| dev | DEV | None (auto-login as DEMO_USER) |
| standard | STANDARD | admin / h!iK*4WzdRehyd6ej^xHjZTPruuY |
| enterprise | OIDC | External IdP |
| govcloud | CAC | Smart card certificate |
