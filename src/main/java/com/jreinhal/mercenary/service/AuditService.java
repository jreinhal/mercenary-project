...
public class AuditService {
    ...
    public void init() {
        ...
        if (govcloudProfile && !this.failClosed) {
    ...
    public void logAuthSuccess(User user, HttpServletRequest request) {
        AuditEvent event = AuditEvent.create(AuditEvent.EventType.AUTH_SUCCESS, user.getId(), "User authenticated").withUser(user).withRequest(this.clientIpResolver.resolveClientIp(request), request.getHeader("User-Agent"), request.getSession().getId()).withMetadata("authProvider", user.getAuthProvider().name());
        this.log(event);
    }

    public void logAuthFailure(String attemptedUser, String reason, HttpServletRequest request) {
        AuditEvent event = AuditEvent.create(AuditEvent.EventType.AUTH_FAILURE, attemptedUser, "Authentication failed").withRequest(this.clientIpResolver.resolveClientIp(request), request.getHeader("User-Agent"), null).withOutcome(AuditEvent.Outcome.FAILURE, reason);
        this.log(event);
    }
