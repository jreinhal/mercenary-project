        ...
        async function initSectorSelector() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (userContext.displayName) {
                        setText(document.getElementById('operator-id'), userContext.displayName);
            ...
            if (savedSector) {
                ...
                if (optionExists) {
                    sectorSelect.value = savedSector;
                    setText(document.getElementById('active-sector'), savedSector);
                }
            }
            ...
            sectorSelect.addEventListener('change', (e) => {
                ...
                setText(document.getElementById('active-sector'), sector);
        ...
        function updateRightPanel(responseText, sources, confidence, metrics) {
            ...
            const metricsIndicateNoInfo = metrics && (metrics.answerable === false || metrics.errorCode);
        ...
        function renderKnowledgeGraph(sources, entities) {
            ...
            const viewBoxMin = -7.0;
            const viewBoxMax = 7.0;
            ...
            const fixedPositions = [
                { x: 0, y: 5.0, textPos: 'top center' },
                { x: 5.0, y: 0, textPos: 'top center' },
                { x: 0, y: -5.0, textPos: 'bottom center' },
                { x: -5.0, y: 0, textPos: 'bottom center' }
            ];
            ...
            const entityPositions = [
                { x: 3.8, y: 3.8, textPos: 'top right' },
                { x: -3.8, y: -3.8, textPos: 'bottom left' }
            ];
            ...
            function withinBounds(box) {
                return (
                    box.x >= labelBounds.min &&
                    box.y >= labelBounds.min &&
                    (box.x + box.width) <= labelBounds.max &&
            ...
            function getMaxLabelWidth(labelX, anchor) {
                if (anchor === 'start') {
                    return labelBounds.max - labelX;
                }
                if (anchor === 'end') {
                    return labelX - labelBounds.min;
                }
                return Math.min(labelX - labelBounds.min, labelBounds.max - labelX) * 2;
            }

            function fitLabelTextToWidth(labelEl, maxWidth, allowTruncate) {
                if (!labelEl || maxWidth <= 0 || !labelEl.getComputedTextLength) return false;
                let text = labelEl.textContent || '';
                let length = 0;
                try {
                    length = labelEl.getComputedTextLength();
                } catch {
                    return false;
                }
                if (length <= maxWidth) return true;
